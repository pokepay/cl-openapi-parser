...
(defun generate-defpackage-form (package-name export-names &optional (use '(:cl :alexandria)))
...
(defun read-schema-interface-classes ()
  (mapcar #'second
          (rest (uiop:read-file-forms
                 (asdf:system-relative-pathname :openapi-parser "schema-interface.lisp")))))

(defun generate-all-readers (readers)
  (generate-defpackage-form
   :openapi-parser/schema/3/interface
   (append (read-schema-interface-classes)
           (remove-duplicates
            (append
             (json-schema-reader-names)
             (loop :for (class-name slot-name) :in readers
                   :collect (openapi-parser/schema::make-reader-name class-name slot-name)))))
   '(:cl)))
